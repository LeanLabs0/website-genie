...
**Pins are gone.** They were annotations that pointed at the wrong thing. The engine's `x_pct`/`y_pct` are percentages of the full-page capture (up to 15,816px tall) and were applied as CSS percentages of the 340x200 `.shot` frame, which shows that capture `object-fit: cover` from the top. Two failures at once: the coordinate space is wrong (`top: 9.1%` of a 200px frame is 18px, not 9.1% of the page), and the frame only shows the top ~5% of a tall page, so most findings are not in view at all. Rendered markup was `top` 9.1%, 3.3%, 13.1%, 2.2% with every `left` at 50%: four markers stacked inside the site's nav, two overlapping, one for a "hero subhead" sitting on the navigation. Numbering also skipped (1, 3, 4) when a pin's `page` did not match the shown screenshot.
